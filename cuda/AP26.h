...
void waitOnEvent(sclHard hardware, CUevent event){
	...
	while(true){
		...
		if(r == CUDA_SUCCESS){
			...
			const char *msg = 0; cuGetErrorString(r, &msg);
			printf("ERROR: cuEventQuery: %s\n", msg ? msg : "?");
			fprintf(stderr, "ERROR: cuEventQuery: %s\n", msg ? msg : "?");
			cuEventDestroy(event);
			return;
		}
	}
...
// Record a marker on the work queue and poll it until all prior kernels finish,
